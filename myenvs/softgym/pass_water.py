...
class PassWater1DEnv(FluidEnv):
    def __init__(self, observation_mode, action_mode, cached_states_path='pass_water_init_states.pkl', **kwargs):
        ...
        if observation_mode in ['point_cloud', 'key_point']:
            ...
            self.observation_space = Box(low=np.array([-np.inf] * obs_dim), high=np.array([np.inf] * obs_dim),
                                         dtype=np.float32)
    ...
    def set_scene(self, config, states=None):
        ...
        self.glass_states = self.init_glass_state(self.x_center, 0, self.glass_dis_x, self.glass_dis_z, self.height,
                                                  self.border)
        ...
        if states is None:
            ...
            lower_x = self.glass_params['glass_x_center'] - self.glass_params['glass_dis_x'] / 2. + self.glass_params[
                'border']
                ...
                for w_idx in range(len(water_state)):
                    if not in_glass[w_idx]:
                        water_state[w_idx][:3] = lower + fluid_dis * np.array(
                            [cnt % dim_x, cnt // (dim_x * dim_z), (cnt // dim_x) % dim_z])
                        cnt += 1
