...
class VecNormalize(VecEnvWrapper):
    ...
    def __init__(self, venv, training=True, norm_obs=True, norm_reward=True,
        ...
        if isinstance(self.observation_space, spaces.Dict):
            ...
            for key in self.observation_space.spaces.keys():
                self.obs_rms[key] = RunningMeanStd(shape=self.observation_space.spaces[key].shape)
        else:
    ...
    def set_venv(self, venv):
        ...
        if isinstance(self.observation_space, spaces.Dict):
            for key in self.observation_space.spaces.keys():
                if self.obs_rms[key].mean.shape != self.observation_space[key].shape:
        ...
        if self.norm_ob:
            if isinstance(self.observation_space, spaces.Dict):
                for key, value in obs.items():
                    obs[key] = np.clip(
                        (obs[key] - self.obs_rms[key].mean) / np.sqrt(self.obs_rms[key].var + self.epsilon),
